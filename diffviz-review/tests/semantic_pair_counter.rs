...
fn count_semantic_pairs_typescript_react_component_only() {
    ...
    let fixture_json = std::fs::read_to_string(&fixture_path).expect("Failed to read fixture file");
    ...
    let all_fixtures = MockDiffProvider::from_review_fixtures().expect("Failed to load fixtures");
    ...
    let engine = {
        let builder = ReviewEngineBuilder::new(Box::new(all_fixtures), "test_author".to_string());
        builder
            .build(DiffQuery::head_to_unstaged())
            .expect("Failed to build review engine")
    };
    ...
    for (id, reviewable_diff) in reviewable_diffs.iter() {
        ...
        println!(
            "Semantic Pair #{}",
            id.file_path.split('#').last().unwrap_or("?")
        );
        ...
        println!(
            "  Line range: L{}-{}",
            id.line_range.start_line, id.line_range.end_line
        );
        println!();
    }
    ...
    assert!(total > 0, "Should have produced at least one semantic pair");
    println!(
        "✓ Test passed: {} semantic pairs produced for TypeScript React component",
        total
    );
}
