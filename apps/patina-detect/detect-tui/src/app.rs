...
use patina_detect::entities::{Symptom, SymptomId, TriageVerdict};
use patina_detect::persistence::Baseline;
use tui_harness::{Affordance, AppDescription, ELMApp, ModeDoc};

use crate::rendering::{self, RenderingError};
use crate::{
    Result,
...
impl TriageData {
    pub fn from_symptoms(symptoms: Vec<Symptom>) -> Result<Self, RenderingError> {
        let core_diffs = rendering::render_all(symptoms.iter())?;
        Ok(TriageData {
            symptoms,
            core_diffs,
        })
...
fn process_key_event_impl(
    ...
    let fix_instruction_symptom_id = match &ui_state.input_mode {
        crate::state::InputMode::FixInstruction { symptom_id } => Some(symptom_id),
        crate::state::InputMode::Navigation => None,
    };
