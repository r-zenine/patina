...
use patina_detect::entities::Site;
use thiserror::Error;
...
pub fn render_site(site: &Site) -> Result<ReviewableDiff, RenderingError> {
    ...
    let content =
        std::fs::read_to_string(&site.file).map_err(|source| RenderingError::ReadFile {
            path: path_str.clone(),
            source,
        })?;
...
pub fn render_all<'a>(
    symptoms: impl Iterator<Item = &'a patina_detect::entities::Symptom>,
) -> Result<Vec<Vec<ReviewableDiff>>, RenderingError> {
    symptoms
        .map(|symptom| symptom.sites.iter().map(render_site).collect())
        .collect()
}
