...
pub fn render(f: &mut Frame, area: Rect, ui_state: &UiState, review_engine: &ReviewEngine) {
    ...
    match &ui_state.input_mode {
        ...
        InputMode::Instruction { .. } => {
            render_input_status(f, area, "Instruction", "Enter to submit, Esc to cancel", &theme)
        }
...
fn render_navigation_status(
    ...
    let state_paragraph = Paragraph::new(state_info).style(stylesheet::status_bar(theme));
    ...
    let keys_paragraph = Paragraph::new(keybindings).style(stylesheet::status_bar(theme));
...
fn render_input_status(
    ...
    let paragraph = Paragraph::new(status_line).style(stylesheet::status_bar(theme));
