...
/// Build semantic pairs between two semantic trees
///
/// This is the main algorithm that compares semantic units between old and new
/// versions to identify meaningful changes for code review.
pub fn build_semantic_pairs<'a>(
    old_tree: &'a SemanticTree<'a>,
    new_tree: &'a SemanticTree<'a>,
    old_source: &dyn crate::ast_diff::SourceProvider,
    new_source: &dyn crate::ast_diff::SourceProvider,
    parser: &dyn crate::LanguageParser,
) -> Result<Vec<SemanticPair<'a>>, SemanticError> {
    // Ensure trees are for the same language
    if old_tree.language != new_tree.language {
        return Err(SemanticError::LanguageMismatch {
            old: old_tree.language,
            new: new_tree.language,
        });
    }

    let old_units = old_tree.all_units();
    let new_units = new_tree.all_units();
    ...
    for (old_idx, old_unit) in old_units.iter().enumerate() {
        ...
        for (new_idx, new_unit) in new_units.iter().enumerate() {
            ...
            if old_unit.can_pair_with(new_unit, old_source, new_source) {
                // Early optimization: only for Import units which are known to be simple
                let similarity = if matches!(&old_unit.unit_type, SemanticUnitType::Import { .. })
                    && old_unit.is_semantically_identical(new_unit, old_source, new_source)
                {
                    // Import units that are semantically identical don't need expensive comparison
                    SemanticSimilarity::identical()
                } else {
                    // Calculate similarity using language-specific comparison for all other units
                    parser.compare_semantic_units(old_unit, new_unit, old_source, new_source)
                };

                pairs.push(SemanticPair::Matched {
                    old_unit,
                    new_unit,
                    similarity,
                });
    ...
    // Phase 2: Mark remaining as additions/deletions
    for (old_idx, old_unit) in old_units.iter().enumerate() {
        if !used_old[old_idx] {
...
/// Helper function to mark a node and all its children as used
/// This prevents children of matched parents from creating separate pairs
fn mark_node_and_children_as_used<'a>(
    node: &'a SemanticNode<'a>,
    all_units: &[&'a SemanticNode<'a>],
    used_flags: &mut [bool],
) {
    // Mark this node as used
    if let Some(idx) = all_units.iter().position(|&n| std::ptr::eq(n, node)) {
        used_flags[idx] = true;
    }
...
/// Build semantic pairs with exhaustive coverage tracking
///
/// This function ensures ALL semantic nodes are accounted for and provides
/// detailed statistics about coverage.
pub fn build_semantic_pairs_with_coverage<'a>(
    old_tree: &'a SemanticTree<'a>,
    new_tree: &'a SemanticTree<'a>,
    old_source: &dyn crate::ast_diff::SourceProvider,
    new_source: &dyn crate::ast_diff::SourceProvider,
    parser: &dyn crate::LanguageParser,
) -> Result<(Vec<SemanticPair<'a>>, CoverageStats), SemanticError> {
    // Ensure trees are for the same language
    if old_tree.language != new_tree.language {
        return Err(SemanticError::LanguageMismatch {
            old: old_tree.language,
    ...
    // Use all_units() for complete coverage (no filtering)
    let old_units = old_tree.all_units();
    let new_units = new_tree.all_units();
    ...
    // Phase 1: Exact name matches (same as original algorithm)
    for (old_idx, old_unit) in old_units.iter().enumerate() {
        if used_old[old_idx] {
            continue;
        }
        ...
        for (new_idx, new_unit) in new_units.iter().enumerate() {
            ...
            if old_unit.can_pair_with(new_unit, old_source, new_source) {
                ...
                // Mark the matched nodes and ALL their children as used
                // This prevents children from creating separate deletion/addition pairs
    ...
    // Phase 2: Only truly orphaned nodes become additions/deletions
    for (old_idx, old_unit) in old_units.iter().enumerate() {
        if !used_old[old_idx] {
            pairs.push(SemanticPair::Deletion { unit: old_unit });
        }
    ...
    let stats = CoverageStats {
        total_old_nodes: old_units.len(),
        total_new_nodes: new_units.len(),
        matched_pairs: pairs
            .iter()
            .filter(|p| matches!(p, SemanticPair::Matched { .. }))
            .count(),
        deletions: pairs
            .iter()
            .filter(|p| matches!(p, SemanticPair::Deletion { .. }))
            .count(),
        additions: pairs
            .iter()
            .filter(|p| matches!(p, SemanticPair::Addition { .. }))
            .count(),
    };

    // Verify exhaustive coverage with assertions
    assert_eq!(
        stats.matched_pairs + stats.deletions,
        old_units.len(),
        "Not all old nodes covered! Expected: {}, Got: {}",
        old_units.len(),
        stats.matched_pairs + stats.deletions
    );
    assert_eq!(
        stats.matched_pairs + stats.additions,
        new_units.len(),
        "Not all new nodes covered! Expected: {}, Got: {}",
        new_units.len(),
        stats.matched_pairs + stats.additions
    );
    assert!(stats.is_exhaustive(), "Coverage verification failed!");

    Ok((pairs, stats))
}

/// Calculate semantic similarity between two nodes based on their characteristics
#[allow(dead_code)]
fn calculate_semantic_similarity<'a>(
    old_unit: &SemanticNode<'a>,
    new_unit: &SemanticNode<'a>,
) -> SemanticSimilarity {
    use SemanticUnitType::*;

    // First check if they're the same semantic type
    if std::mem::discriminant(&old_unit.unit_type) != std::mem::discriminant(&new_unit.unit_type) {
        return SemanticSimilarity::unrelated();
    }

    // Check name changes
    let name_changed = check_name_changed(old_unit, new_unit);

    match (&old_unit.unit_type, &new_unit.unit_type) {
        (
            Callable {
            ...
            let signature_changed =
                old_params != new_params || old_ret != new_ret || old_async != new_async;
            let structural_changed = old_vis != new_vis;
            let signature_similarity = calculate_signature_similarity(
                *old_params,
                *new_params,
                old_ret,
                new_ret,
            ...
            // Check for body changes by comparing node text (simplified heuristic)
            let body_changed = check_body_changed(old_unit, new_unit);
            ...
            SemanticSimilarity {
                ...
                body_changed,
                structural_changed,
                match_confidence: if signature_changed {
                    signature_similarity
                } else {
                    1.0
                },
                confidence_metrics: SimilarityMetrics {
                    name_similarity: if name_changed { 0.0 } else { 1.0 },
                    signature_similarity,
                    body_similarity: if body_changed { 0.8 } else { 1.0 },
                    structural_similarity: if structural_changed { 0.7 } else { 1.0 },
                },
            }
        }
        ...
        (
            ...
            let signature_changed = old_fields != new_fields || old_generic != new_generic;
            let structural_changed = old_vis != new_vis;
            let body_changed = check_body_changed(old_unit, new_unit);

            let structural_similarity = if signature_changed { 0.8 } else { 1.0 };
            ...
            SemanticSimilarity {
                ...
                body_changed,
                structural_changed,
                match_confidence: structural_similarity,
                confidence_metrics: SimilarityMetrics {
                    name_similarity: if name_changed { 0.0 } else { 1.0 },
                    signature_similarity: structural_similarity,
                    body_similarity: if body_changed { 0.8 } else { 1.0 },
        ...
        | (Module { .. }, Module { .. })
        | (Unknown { .. }, Unknown { .. }) => {
            // For these types, detect changes more simply
            let body_changed = check_body_changed(old_unit, new_unit);
        ...
        _ => {
            // Different semantic types or unmatched patterns
...
fn calculate_signature_similarity(
    ...
    let param_similarity = if old_params == new_params {
        1.0
    } else {
        1.0 - (old_params as f32 - new_params as f32).abs()
            / (old_params.max(new_params) as f32).max(1.0)
    };

    let return_similarity = match (old_return, new_return) {
        (None, None) => 1.0,
        (Some(old), Some(new)) if old == new => 1.0,
        (Some(_), Some(_)) => 0.5, // different return types
        _ => 0.3,                  // one has return type, other doesn't
    };

    let async_similarity = if old_async == new_async { 1.0 } else { 0.6 };

    (param_similarity + return_similarity + async_similarity) / 3.0
}

/// Check if name changed between two semantic units
#[allow(dead_code)]
fn check_name_changed<'a>(_old_unit: &SemanticNode<'a>, _new_unit: &SemanticNode<'a>) -> bool {
    // For now, assume names don't change if units are paired (they should be paired by name)
    // TODO: Implement proper name comparison using source text
    false
}

/// Check if body changed between two semantic units (simplified heuristic)
#[allow(dead_code)]
fn check_body_changed<'a>(old_unit: &SemanticNode<'a>, new_unit: &SemanticNode<'a>) -> bool {
    // Simple heuristic: compare node byte ranges
    let old_range = old_unit.tree_sitter_node.byte_range();
    let new_range = new_unit.tree_sitter_node.byte_range();

    // If sizes are significantly different, assume body changed
    let old_size = old_range.end - old_range.start;
    let new_size = new_range.end - new_range.start;

    if old_size != new_size {
        let size_diff_ratio =
            (old_size as f32 - new_size as f32).abs() / old_size.max(new_size) as f32;
        size_diff_ratio > 0.1 // More than 10% size difference suggests body change
    } else {
        // Same size, but could still be different content
        // In a real implementation, we'd compare AST structure or content hashes
        false
    }
}
...
mod tests {
    ...
    #[test]
    fn test_language_mismatch_error() {
        use crate::parsers::RustParser;
        use tree_sitter::Parser;
        ...
        let rust_tree = SemanticTree::new(unit.clone(), ProgrammingLanguage::Rust);
        let python_tree = SemanticTree::new(unit, ProgrammingLanguage::Python);

        let dummy_source = crate::ast_diff::SourceCode::new("");
        let result = build_semantic_pairs(
            &rust_tree,
            &python_tree,
            &dummy_source,
            &dummy_source,
        ...
        assert!(matches!(
            result,
            Err(SemanticError::LanguageMismatch { .. })
        ));
    }
    ...
    fn test_exhaustive_coverage() {
        ...
        let new_code = "fn hello() { println!(\"world\"); }";

        let old_tree = ts_parser.parse(old_code, None).unwrap();
        let new_tree = ts_parser.parse(new_code, None).unwrap();

        let old_semantic_tree = parser_impl
            .build_semantic_tree(&old_tree, old_code)
            .unwrap();
        let new_semantic_tree = parser_impl
            .build_semantic_tree(&new_tree, new_code)
            .unwrap();

        let old_source = crate::ast_diff::SourceCode::new(old_code);
        let new_source = crate::ast_diff::SourceCode::new(new_code);

        // Test new coverage-aware function
        let (pairs, stats) = build_semantic_pairs_with_coverage(
            &old_semantic_tree,
            &new_semantic_tree,
            &old_source,
            &new_source,
        ...
        assert!(stats.is_exhaustive(), "Coverage should be 100%");
        assert!(stats.total_old_nodes > 0, "Should have old nodes");
        assert!(stats.total_new_nodes > 0, "Should have new nodes");

        // Verify all pairs account for all nodes
        let total_pairs = pairs.len();
        assert_eq!(
            stats.matched_pairs + stats.additions + stats.deletions,
            total_pairs
        );

        // Show detailed coverage breakdown
        let (matched_pct, deleted_pct, added_pct) = stats.coverage_breakdown();
        println!(
            "Coverage breakdown: {matched_pct:.1}% matched, {deleted_pct:.1}% deleted, {added_pct:.1}% added"
        );
        println!("Total coverage: {:.1}%", stats.coverage_percentage());
    }

    #[test]
    fn test_child_coverage_marking() {
        use crate::parsers::RustParser;
        use tree_sitter::Parser;
        ...
        // More complex code that should create parent-child relationships
        let old_code = "
        struct Config {
            name: String,
            value: i32,
        ...
        impl Config {
            fn new(name: String) -> Self {
                Config { name, value: 0, enabled: true }
            }
        }";
        ...
        // Test coverage-aware pairing
        let (_pairs, stats) = build_semantic_pairs_with_coverage(
            &old_semantic_tree,
            &new_semantic_tree,
            &old_source,
            &new_source,
        ...
        let (matched_pct, deleted_pct, added_pct) = stats.coverage_breakdown();
        println!(
            "Child coverage breakdown: {matched_pct:.1}% matched, {deleted_pct:.1}% deleted, {added_pct:.1}% added"
        );
        println!("Total coverage: {:.1}%", stats.coverage_percentage());

        // Verify that our child coverage marking is working:
        // - We should have meaningful matches (struct and function)
        // - Only root modules should be unmatched (they have different content)
        assert!(
            stats.matched_pairs >= 2,
            "Should have matched the struct and function"
        );
        assert_eq!(
            stats.deletions, 1,
            "Should have 1 deletion (old root module)"
        );
        assert_eq!(
            stats.additions, 1,
            "Should have 1 addition (new root module)"
        );
    }
